...
Options:
    ...
    --output        Clean output_dir directory (standard output location)
    --benchmarks    Clean benchmark results
    --all           Clean everything (temp, reports, output_dir, benchmarks)
...
def parse_args():
    ...
    cleanup_group.add_argument('--output', action='store_true',
                               help='Clean output_dir directory (standard output location)')
    ...
    cleanup_group.add_argument('--all', action='store_true',
                               help='Clean everything (temp, reports, output_dir, benchmarks)')
...
def clean_output(project_root, dry_run=False, days_old=0):
    ...
    logger.info("Cleaning output_dir directory...")

    # Check the output directory path
    output_dir = os.path.join(project_root, "output_dir")
    
    # Check for legacy output directory
    legacy_output_dir = os.path.join(project_root, "output")
    
    cleaned = 0
    
    # Clean standard output_dir
    if os.path.exists(output_dir):
        logger.info(f"Cleaning files in output_dir: {output_dir}")
        ...
        logger.info("Standard output_dir directory does not exist")
        
    # Handle legacy output directory if it exists
    if os.path.exists(legacy_output_dir):
        logger.info(f"Found legacy 'output' directory: {legacy_output_dir}")
        logger.info(f"The project now standardizes on 'output_dir' instead of 'output'")
        
        if dry_run:
            logger.info(f"Would remove legacy output directory: {legacy_output_dir}")
        else:
            try:
                shutil.rmtree(legacy_output_dir)
                logger.info(f"Removed legacy output directory: {legacy_output_dir}")
                cleaned += 1
            except Exception as e:
                logger.error(f"Failed to remove legacy output directory: {e}")
...
def should_remove_for_repo(path):
    ...
    essential_dirs = {
        ...
        'output_dir', 'reports',  # Keep these directories but clean their contents separately
    }
...
def prepare_repo(project_root, dry_run=False, keep_benchmarks=False):
    ...
    for dir_name in ['output_dir', 'reports']:
        ...
        if not readme_path.exists() and not dry_run:
            with open(readme_path, 'w') as f:
                content = "# Output Directory\n\nThis directory is the default output location for JPEG2000 files generated by jp2forge.\n" if dir_name == 'output_dir' else f"# {dir_name.capitalize()} Directory\n\nThis directory contains {dir_name} generated by JP2Forge.\n"
                f.write(content)
